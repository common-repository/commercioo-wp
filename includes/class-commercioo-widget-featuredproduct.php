 ...
 */
class Commercioo_Widget_Featured_Product extends WP_Widget {
	...
	function __construct() {
        $widget_ops = array(
            'classname'                   => 'commercioo_widget_featured_product',
            'description'                 => __( 'This is Commercioo Featured Product Widget', 'wpb_widget_domain' ),
            'customize_selective_refresh' => true,
        );
		parent::__construct (
			'Commercioo_Widget_Featured_Product', 
			__('Featured Products by Commercioo', 'Commercioo_Widget_Featured_Product_domain'),
            $widget_ops
		);
		$this->init();
	}
	...
	public function widget( $args, $instance ) {
        ...
        $default_title = __( 'Featured Product' );

        $title         = ( ! empty( $instance['title'] ) ) ? $instance['title'] : $default_title;
        /** This filter is documented in wp-includes/widgets/class-wp-widget-pages.php */
        $title = apply_filters( 'widget_title', $title, $instance, $this->id_base );

        $number = ( ! empty( $instance['number_featured_prod'] ) ) ? absint( $instance['number_featured_prod'] ) : 4;
        if ( ! $number ) {
            $number = 4;
        }
        $prod_featured = apply_filters("comm_calculate_featured_product",$number);
        ...
        $maxProd = 4;
        $output .= "<div class='row'>";
        if(is_array($prod_featured) && $prod_featured){
            foreach ($prod_featured as $pVal) {
	...
	public function form( $instance ) {
        $title  = isset( $instance['title'] ) ? $instance['title'] : 'Featured Product';
        $number = isset( $instance['number_featured_prod'] ) ? absint( $instance['number_featured_prod'] ) : 4;
        ?>
        <p>
        ...
        <p>
            <label for="<?php echo esc_attr($this->get_field_id( 'number_featured_prod' )); ?>"><?php _e( 'Number of product(s) to show:' );
            ?></label>
            <input class="tiny-text" id="<?php echo esc_attr($this->get_field_id( 'number_featured_prod' )); ?>" name="<?php echo
            esc_attr($this->get_field_name( 'number_featured_prod' )); ?>" type="number" step="1" min="1" max="4" value="<?php echo esc_attr($number);
            ?>" size="3" />
        </p>
        <?php
	}
	...
	public function update( $new_instance, $old_instance ) {
        ...
        $instance['number_featured_prod'] = absint( $new_instance['number_featured_prod'] );
        return $instance;
	}
	...
	public function wpb_load_widget() {
		register_widget( 'Commercioo_Widget_Featured_Product' );
	}
